...
/**
 * Handles the change in measurement type (imperial or metric) by dynamically updating
 * the options in the corresponding dropdown menus. It clears existing options and
 * repopulates the dropdowns based on the selected measurement type.
 * 
 * @param {string} measurementType - The type of measurement selected ('imperial' or 'metric').
 * @param {string} cellId - The unique identifier for the cell that is being updated.
 */
function handleMeasurementTypeChange(measurementType, cellId) {
	const measurementTypeSelect = document.getElementById(`measurement-type-${cellId}`);
	const heightDropdownContainer = document.getElementById(`height-dropdowns-container-${cellId}`);
	const weightDropdownContainer = document.getElementById(`weight-dropdowns-container-${cellId}`);

	if (!measurementTypeSelect || !heightDropdownContainer || !weightDropdownContainer) { 
		console.error(`Error: Missing required element(s) for measurement type change. 
			Check if measurement-type-${cellId}, height-dropdowns-container-${cellId}, 
			and weight-dropdowns-container-${cellId} exist in the DOM.`);
		return;
	}

	/* Clear existing options */
		heightDropdownContainer.innerHTML = '';
		weightDropdownContainer.innerHTML = '';

	if (measurementType === 'imperial') {
		createDropdown(
			weightDropdownContainer, 
			`select-weight-${cellId}`, 
			'drop-down', 
			'pet-weight', 
			generateOptions(0, 999, 'lb'), 
			Object.assign(document.createElement('option'), { value: '', selected: true, disabled: true, textContent: 'Weight (lb)' })
		);
		...
		createDropdown(
			...
			'drop-down', 
			`height-1`,  
			generateOptions(0, 30, 'ft'), 
			Object.assign(document.createElement('option'), { value: '', selected: true, disabled: true, textContent: 'Height (ft)' })
		);
		
		createDropdown(heightDropdownContainer, 
			`in-select-${cellId}`, 
			'drop-down', 
			`select-2`, 
			generateOptions(0, 11, ' in'), 
			Object.assign(document.createElement('option'), { value: '', selected: true, disabled: true, textContent: 'Height (in)' })
		);
		...
		createDropdown(
			...
			'drop-down', 
			'pet-weight', 
			generateOptions(0, 999, 'kg'), 
			Object.assign(document.createElement('option'), { value: '', selected: true, disabled: true, textContent: 'Weight (kg)' })
		);
		...
		createDropdown(
			...
			'drop-down', 
			`height-1`, 
			generateOptions(0, 30, 'm'), 
			Object.assign(document.createElement('option'), { value: '', selected: true, disabled: true, textContent: 'Height (m)' })
		);
		...
		createDropdown(
			...
			'drop-down', 
			`height-2`, 
			generateOptions(0, 100, 'cm'), 
			Object.assign(document.createElement('option'), { value: '', selected: true, disabled: true, textContent: 'Height (cm)' })

		);
	}

	heightDropdownContainer.disabled = false;
}

/**
 * Creates a dropdown (<select> element) with specified attributes and appends it to a given container element.
 * The dropdown is populated with the provided options.
 * 
 * @param {HTMLElement} container - The parent element to which the dropdown will be appended.
 ...
 * @param {HTMLOptionElement[]} options - An array of <option> elements to populate the dropdown.
 * @param {HTMLOptionElement} placeHolder - The placeholder <option>.
 */
function createDropdown(container, id, className, name, options, placeHolder) {
	const selector = Object.assign(document.createElement('select'), { id, className, name });
...
/**
 * Generates an array of <option> elements for a dropdown menu. Each option represents a value
 * within the specified range and includes a unit suffix in its display text.
 * 
 * @param {number} min - The minimum value for the options.
 * @param {number} max - The maximum value for the options.
 * @param {string} unit - The unit suffix to be appended to the option text (e.g., 'ft', 'cm').
 * @returns {HTMLOptionElement[]} - An array of <option> elements ready to be appended to a dropdown.
 */
