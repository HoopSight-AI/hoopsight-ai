	...
	window.addEventListener('scroll', function() { // This line creates the event listener that will help track the user's scrolling
		...
		*/
		...
		if (navBarTop < -codedNavBarHeight - 5) {
			navbar.style.top = -codedNavBarHeight - 5;
		} else if (navBarTop > 0) {
			navbar.style.top = 0;
		}
...
function adjustTopSpacing() {
	...
	spacingElement.style.marginTop = parseFloat(navBarHeight) + 2; // Set the margin-top of the top-spacing element to the height of the nav-bar. This line will style using px
}
